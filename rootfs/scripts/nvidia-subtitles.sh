...
if [ $codec_name = "AVC" ] ; then
    if [ $subs = "DVB" ] ; then
	    ffmpeg -hwaccel cuvid -c:v h264_cuvid -deint 2 -drop_second_field 1 -surfaces 10 -i "$1" -filter_complex "hwdownload,format=nv12,format=yuv420p" -c:v hevc_nvenc -preset:v hp -level:v 5.0 -rc:v vbr_hq -rc-lookahead:v 32 -brand mp42 -ac 2 -c:a libfdk_aac -b:a 192k -c:s copy "$map/$mkv"
	else
